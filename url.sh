...
JOB_ID=$(curl -s --header "Authorization: Bearer $TOKEN" --header "Content-Type: application/json" -X POST -d '{ "initialData": {"url": "https://cristinel-test.msqa.sugarcrm.com" }}' "https://$ENV.virtuoso.qa/api/goals/$GOAL_ID/execute?envelope=false" | jq -r .id)
